...
export async function POST(request: Request) {
  ...
  // Enforce: ops only receives "補完後" data (no floating parts).
  // The client UI should already gate this, but we validate again on the server.
  const prep = analyzePrintPrepSupport(baseSet, supSet);
    ...
    return NextResponse.json(
      {
        ...
        message:
          `モデルに浮動するブロックがあります（${prep.componentCount}パーツ）。` +
  ...
  // Normalize to unique keys (avoid duplicates in JSONB)
  const blocksNorm = Array.from(baseSet);
  const supportBlocksNorm = Array.from(supSet);
  ...
  const insertOrder: any = {
    ...
    model_data: {
      blocks: blocksNorm,
      supportBlocks: supportBlocksNorm,
      scaleSetting,
      // for convenience
      mmPerUnit: resolved.mmPerUnit,
