...
作品はサーバーへ送信せず、右上の「書き出し」から `.qbu`、色別STL、manifestをまとめたZIPとして端末へダウンロードします。
...
- ZIP内は `<作品名>/0001/0001.qbu`、`0001_<作品名>_<色>.stl`、`<作品名>/manifest.csv`
- クラス・参加コードを持たない公開版では、WS Dashboard互換のフォルダ番号として `0001` を使用
- 「色の切り替え」と「ユーザー設定」の間のインポートボタンから、公開版・WS版の平文JSON `.qbu` を読み込み可能
- インポート時は形式、座標、色、重複、グリッド範囲、最大ブロック数、5MB上限を検証
